...
=========================================================*/
function ariclaw_opt( $id = null, $default = '' ) {
...
function ariclaw_site_icon(){
	if ( ! has_site_icon() ) {
		$html = '';
		$icon_path = ARICLAW_DIR_ASSETS_URI . 'img/favicon.png';
...
=========================================================*/
function ariclaw_meta( $id = '' ){
    
    $value = get_post_meta( get_the_ID(), '_ariclaw_'.$id, true );
...
/*=========================================================
	Blog Date Permalink
=========================================================*/
function ariclaw_blog_date_permalink(){
...
/*========================================================
	Blog Excerpt Length
========================================================*/
if ( ! function_exists( 'ariclaw_excerpt_length' ) ) {
	function ariclaw_excerpt_length( $limit = 30 ) {
...
==========================================================*/
if ( ! function_exists( 'ariclaw_posted_comments' ) ) {
    function ariclaw_posted_comments(){
        
        $comments_num = get_comments_number();
        if( comments_open() ){
            if( $comments_num == 0 ){
                $comments = esc_html__('No Comments','ariclaw');
            } elseif ( $comments_num > 1 ){
                $comments= $comments_num . esc_html__(' Comments','ariclaw');
            } else {
                $comments = esc_html__( '1 Comment','ariclaw' );
            }
            $comments = '<i class="ti-comments"></i>'. $comments;
        } else {
            $comments = esc_html__( 'Comments are closed', 'ariclaw' );
        }
...
/*===================================================
	Post embedded media
===================================================*/
function ariclaw_embedded_media( $type = array() ){
...
/*===================================================
	WP post link pages
====================================================*/
function ariclaw_link_pages(){
    wp_link_pages( array(
    'before'      => '<div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages:', 'ariclaw' ) . '</span>',
    'after'       => '</div>',
    'link_before' => '<span>',
    'link_after'  => '</span>',
    'pagelink'    => '<span class="screen-reader-text">' . esc_html__( 'Page', 'ariclaw' ) . ' </span>%',
...
/*====================================================
	Theme logo
====================================================*/
function ariclaw_theme_logo( $class = '' ) {

    $siteUrl = home_url('/');
    // site logo
		
	$custom_logo_id = get_theme_mod( 'custom_logo' );
	$imageUrl = wp_get_attachment_image_src( $custom_logo_id , 'ariclaw_logo_178x62' );
	
	if( !empty( $imageUrl[0] ) ){
		$siteLogo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'.esc_url( $imageUrl[0] ).'" alt="ariclaw logo"></a>';
	}else{
...
/*================================================
    Page Title Bar
================================================*/
function ariclaw_page_titlebar() {
	if ( ! is_page_template( 'template-builder.php' ) ) {
        ...
        <section class="hero-banner">
            <div class="container">
				...
				<?php
				if ( is_category() ) {
					single_cat_title( __('Category: ', 'ariclaw') );
					...
					echo esc_html__( 'Search for: ', 'ariclaw' ) . get_search_query();

				} elseif ( ! ( is_404() ) && ( is_single() ) || ( is_page() ) ) {
					echo  get_the_title();

				} elseif ( is_home() ) {
					echo esc_html__( 'Blog', 'ariclaw' );

				} elseif ( is_404() ) {
					echo esc_html__( '404 error', 'ariclaw' );
                ...
                <nav aria-label="breadcrumb" class="banner-breadcrumb">
					<?php
					if ( function_exists( 'ariclaw_breadcrumbs' ) ) {
						ariclaw_breadcrumbs();
					}
					?>
                </nav>
...
=================================================*/
function ariclaw_pull_right( $id = '', $condation ){
...
/*======================================================
	Inline Background
======================================================*/
function ariclaw_inline_bg_img( $bgUrl ){
    $bg = '';
...
/*======================================================
	Blog Category
======================================================*/
function ariclaw_featured_post_cat(){
...
========================================================*/
function ariclaw_sidebar_opt(){

    $sidebarOpt = ariclaw_opt( 'ariclaw_blog_layout' );
    $sidebar = '1';
...
/**================================================
	Themify Icon
 =================================================*/
function ariclaw_themify_icon(){
    return[
        'cap'     => __('Icon Cap', 'ariclaw'),
        'bag'     => __('Icon Bag', 'ariclaw'),
        'shirt'   => __('Icon T-shirt', 'ariclaw'),
        'cafe'    => __('Icon Cafe', 'ariclaw'),
    ];
}
...
============================================================*/
function ariclaw_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'ariclaw_contact7_form_content', 10, 2 );
...
/*============================================================
	Pagination
=============================================================*/
function ariclaw_blog_pagination(){
	echo '<nav class="blog-pagination justify-content-center d-flex">';
        the_posts_pagination(
            array(
                'mid_size'  => 2,
                'prev_text' => __( '<span class="ti-angle-left"></span>', 'ariclaw' ),
                'next_text' => __( '<span class="ti-angle-right"></span>', 'ariclaw' ),
                'screen_reader_text' => ' '
            )
        );
	echo '</nav>';
...
=============================================================*/
if( ! function_exists('ariclaw_blog_single_post_navigation') ) {
	function ariclaw_blog_single_post_navigation() {
				...
				<div class="row">
					<div class="col-lg-6 col-md-6 col-12 nav-left flex-row d-flex justify-content-start align-items-center">
						...
						if( get_next_post_link() ){
							...
							if( has_post_thumbnail() ){
								...
								<div class="thumb">
									<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
										<?php echo get_the_post_thumbnail( absint( $nextPost->ID ), 'ariclaw_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
									</a>
								</div>
								<?php
							} ?>
							...
							<div class="detials">
								<p><?php echo esc_html__( 'Prev Post', 'ariclaw' ); ?></p>
								<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
					...
					<div class="col-lg-6 col-md-6 col-12 nav-right flex-row d-flex justify-content-end align-items-center">
						...
						if( get_previous_post_link() ){
							...
							?>
							<div class="detials">
								<p><?php echo esc_html__( 'Next Post', 'ariclaw' ); ?></p>
							...
							<div class="thumb">
								<a href="<?php the_permalink(  absint( $prevPost->ID )  ) ?>">
									<?php echo get_the_post_thumbnail( absint( $prevPost->ID ), 'ariclaw_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
								</a>
							</div>
							<?php
						} ?>
...
/*===================================================
 Ariclaw Comment Template Callback
 ====================================================*/
function ariclaw_comment_callback( $comment, $args, $depth ) {
				...
				<div class="desc">
					...
					<div class="d-flex justify-content-between">
						<div class="d-flex align-items-center">
							<h5 class="comment_author"><?php printf( __( '<span class="comment-author-name">%s</span> ', 'ariclaw' ), get_comment_author_link() ); ?></h5>
							<p class="date"><?php printf( __('%1$s at %2$s', 'ariclaw'), get_comment_date(),  get_comment_time() ); ?><?php edit_comment_link( esc_html__( '(Edit)', 'ariclaw' ), '  ', '' ); ?> </p>
							<?php if ( $comment->comment_approved == '0' ) : ?>
								<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'ariclaw' ); ?></em>
								<br>
							<?php endif; ?>
						</div>
...
add_filter('comment_reply_link', 'ariclaw_replace_reply_link_class');
function ariclaw_replace_reply_link_class( $class ) {
...
===========================================================*/
function ariclaw_latest_blog( $pNumber = 3, $post_exc = 35, $post_order = 'DESC' ){
		...
		<div class="col-lg-4 col-sm-6">
			<div class="single_offer_part">
				<div class="single_offer">
					<?php
						if( has_post_thumbnail() ){
							the_post_thumbnail( 'ariclaw_latest_blog_360x458', ['alt' => get_the_title() ] );
						}
					?>
					<div class="hover_text">
						<div class="single-home-blog">
							<?php echo ariclaw_featured_post_cat(); ?>
							<span class="time"> <i class="ti-time"></i> <?php the_time('F j, Y') ?></span>
							<a href="<?php the_permalink(); ?>">
								<h5 class="card-title"><?php the_title(); ?></h5>
							</a>
							<p><?php echo ariclaw_excerpt_length( $post_exc ) ?></p>
							<ul class="home-blog-post-like-comments">
								<li><?php echo get_simple_likes_button( get_the_ID() );?></li>
								<li><?php echo ariclaw_posted_comments();?></li>
							</ul>
						</div>
					</div>
				</div>
...
/*=========================================================
    Share Button Code
===========================================================*/
function ariclaw_social_sharing_buttons( $ulClass = '', $tagLine = '' ) {
